...
int WFQ_weights[8]= {1 , 1 , 1 , 1 , 1 , 1 , 1 , 1 };
...
void roundRobinScheduler(){ //we have reused the code of WFQ because round robin is nothing but WFQ with all weights same say 1 
        ...
        for (int o = 0; o < NUM_PORTS; o++) {  // Lock shared resources
            for(int i = 0 ; i < WFQ_weights[o] ; i++){
                    if (!inputQueue[o].empty()) {
                        ...
                        cout << "Round Robin: Input " << o << " to Output " << o << " for Packet " << pkt.id << endl;
                        ...
                        pkt.processingTime = (bps-pkt.arrivalTime + pkt.size/processing_rate);
...
void clearOutput() {
    while (!terminateThreads) {
        for (int i = 0; i < NUM_PORTS; i++) { // Lock shared resources
...
int main() {
    ...
    thread scheduler(roundRobinScheduler);
