...
int runDVR_poisioned_reverse(Router Routers[], int N){
    bool change = true;
    int itr = 0;
        ...
        for(int i=1; i<=N; i++){
            for(int j=1; j<=N; j++){
                ...
                for(int k=0; k<Routers[i].Neighbour.size(); k++){
                    ...
                    int if_needed = Sent[Routers[i].Neighbour[k].first][j].first;
                    
                    if(Sent[Routers[i].Neighbour[k].first][j].second==i){
                        Sent[Routers[i].Neighbour[k].first][j].first = inf ; // in poisioning it sends the neighbour infinity value if the next hop from the neighbour is the corresponding router itself   
                    } // so here to simluate that we make the value infinity 
                    ...
                    Sent[Routers[i].Neighbour[k].first][j].first = if_needed ;
                }
...
int main(){
    ...
    //DVR algorithm
    int iter = runDVR_poisioned_reverse(Routers, N);
    ...
    int ret = runDVR_poisioned_reverse(Routers, N);
